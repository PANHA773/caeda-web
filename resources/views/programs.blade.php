...
<div class="bg-white shadow-lg -mt-8 relative z-10 mx-4 md:mx-8 rounded-2xl">
    <div class="max-w-7xl mx-auto grid grid-cols-2 md:grid-cols-4 divide-x divide-gray-200">
        <div class="text-center py-6">
            <div class="text-3xl md:text-4xl font-bold text-blue-900">{{ $programs->count() }}+</div>
            <div class="text-gray-600 mt-1">Programs</div>
        </div>
        <div class="text-center py-6">
            <div class="text-3xl md:text-4xl font-bold text-blue-900">{{ number_format($programs->sum('students')) }}</div>
            <div class="text-gray-600 mt-1">Students Enrolled</div>
        </div>
        <div class="text-center py-6">
            <div class="text-3xl md:text-4xl font-bold text-blue-900">
                @if($programs->count() > 0)
                    {{ number_format($programs->avg('rating'), 1) }}
                @else
                    0.0
                @endif
            </div>
    ...
    <div class="max-w-7xl mx-auto">
        ...
        <div class="mb-8 flex flex-wrap gap-2">
            ...
            @foreach($categories as $category)
                <button class="category-filter px-4 py-2 bg-gray-200 text-gray-800 rounded-xl hover:bg-gray-300 transition" data-category="{{ strtolower(str_replace(' ', '-', $category)) }}">
                    {{ $category }}
                </button>
            @endforeach
        </div>

        {{-- PROGRAMS GRID (Default View) --}}
        <div id="programs-grid" class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8">
            @forelse($programs as $program)
                ...
                <div class="program-card bg-white rounded-2xl shadow-lg overflow-hidden h-full flex flex-col"
                     data-category="{{ strtolower(str_replace(' ', '-', $program->category ?? 'general')) }}"
                     data-price="{{ $finalPrice }}"
                     data-rating="{{ $program->rating ?? 0 }}"
                     data-featured="{{ $program->is_featured ? '1' : '0' }}">

                    {{-- Program Image --}}
                    <div class="relative h-56 overflow-hidden">
                            <img src="{{ $program->image_url ?? 'https://images.unsplash.com/photo-1516321318423-f06f85e504b3?ixlib=rb-4.0.3&auto=format&fit=crop&w=1200&q=80' }}" 
                        ...
                        @if(!empty($program->badge))
                            <div class="absolute top-4 right-4 {{ $program->badge_color ?? 'bg-blue-600' }} text-white px-3 py-1 rounded-full text-xs font-bold">
                                {{ $program->badge }}
                            </div>
                        @endif
                        ...
                        @if($hasDiscount)
                            <div class="absolute bottom-0 left-0 discount-badge text-white px-4 py-2 font-bold">
                                SAVE ${{ number_format(($program->tuition ?? 0) - $finalPrice) }}
                            </div>
                        @endif
                    </div>
                    ...
                    <div class="p-6 flex-grow flex flex-col">
                        <div class="flex justify-between items-start mb-3">
                            ...
                            <span class="text-xs font-semibold {{ $levelClass }} text-white px-3 py-1 rounded-full">
                                {{ $levelText }}
                            </span>
                        </div>
                        ...
                        <div class="flex items-center text-gray-500 text-sm mb-4">
                            ...
                            <div class="flex items-center">
                                ...
                                <span class="ml-1">({{ number_format($program->students ?? 0) }})</span>
                            </div>
                        </div>
                        ...
                        <div class="flex justify-between items-center mt-auto pt-4 border-t border-gray-100">
                            <div>
                                @if($hasDiscount && ($program->tuition ?? 0) > 0)
                                    <div class="text-sm text-gray-500 line-through">${{ number_format($program->tuition) }}</div>
                                @endif
                        ...
                        @if(isset($program->application_deadline))
                            <div class="mt-4 text-sm text-gray-600 bg-gray-50 p-2 rounded-lg">
                                <i class="far fa-calendar-alt mr-2"></i>
                                Apply by {{ \Carbon\Carbon::parse($program->application_deadline)->format('M d, Y') }}
                            </div>
                        @endif
                    </div>
                </div>
                    ...
                    <div class="md:w-2/3 lg:w-3/4 p-6 flex flex-col">
                        <div class="flex flex-wrap gap-2 mb-3">
                            ...
                            <span class="text-xs font-semibold text-gray-600 bg-gray-100 px-3 py-1 rounded-full">
                                <i class="fas fa-user-graduate mr-1"></i>{{ $program->mode ?? 'Online' }}
                            </span>
                            @if(!empty($program->badge))
                                <span class="text-xs font-semibold {{ $program->badge_color ?? 'bg-blue-600' }} text-white px-3 py-1 rounded-full">
                                    {{ $program->badge }}
                                </span>
                            @endif
                        </div>
                        ...
                        <div class="flex flex-wrap items-center gap-4 mb-4 text-gray-600">
                            ...
                            <div class="flex items-center">
                                ...
                                <span>{{ $program->rating ?? '0.0' }} rating</span>
                            </div>
                        </div>
                            ...
                            <div>
                                @if($hasDiscount && ($program->tuition ?? 0) > 0)
                                    <div class="flex items-center">
                                        ...
                                        <span class="ml-3 text-sm text-gray-500 line-through">${{ number_format($program->tuition) }}</span>
