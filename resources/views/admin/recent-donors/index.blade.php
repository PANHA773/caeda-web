        ...
        <div class="bg-white p-5 rounded-2xl border border-gray-100 shadow-sm">
            <div class="flex items-center justify-between">
                <div>
                    ...
                    <p class="text-2xl font-bold text-gray-900 mt-1">{{ $donors->count() }}</p>
                </div>
        ...
        <div class="bg-white p-5 rounded-2xl border border-gray-100 shadow-sm">
            <div class="flex items-center justify-between">
                <div>
                    ...
                    <p class="text-2xl font-bold text-gray-900 mt-1">${{ number_format($donors->sum('amount'), 2) }}</p>
                </div>
        ...
        <div class="bg-white p-5 rounded-2xl border border-gray-100 shadow-sm">
            <div class="flex items-center justify-between">
                <div>
                    ...
                    <p class="text-2xl font-bold text-gray-900 mt-1">${{ $donors->count() > 0 ? number_format($donors->avg('amount'), 2) : '0.00' }}</p>
                </div>
        ...
        <div class="bg-white p-5 rounded-2xl border border-gray-100 shadow-sm">
            <div class="flex items-center justify-between">
                <div>
                    ...
                    <p class="text-2xl font-bold text-gray-900 mt-1">{{ $donors->where('is_recurring', true)->count() }}</p>
                </div>
    ...
    <div class="bg-white rounded-2xl border border-gray-200 shadow-sm overflow-hidden">
                    ...
                    <tr class="hover:bg-gray-50 transition-colors duration-150">
                        ...
                        <td class="px-6 py-4">
                            <div class="flex items-center gap-4">
                                ...
                                <div>
                                    <div class="flex items-center gap-2">
                                        <h3 class="font-semibold text-gray-900">{{ $donor->first_name }} {{ $donor->last_name }}</h3>
                        ...
                        <td class="px-6 py-4">
                            <div class="flex items-center gap-2">
                                ...
                                <div>
                                    <span class="font-bold text-gray-900 text-lg">${{ number_format($donor->amount, 2) }}</span>
                                    @if($donor->phone)
                        ...
                        <td class="px-6 py-4">
                            <div class="flex items-center gap-2">
                                ...
                                @endphp
                                <svg class="w-5 h-5 {{ $paymentIcon }}" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                                    @if($donor->payment_method == 'credit_card')
                                        <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M3 10h18M7 15h1m4 0h1m-7 4h12a3 3 0 003-3V8a3 3 0 00-3-3H6a3 3 0 00-3 3v8a3 3 0 003 3z"/>
                                    @elseif($donor->payment_method == 'paypal')
                                ...
                                <span class="text-gray-700 capitalize">{{ str_replace('_', ' ', $donor->payment_method) }}</span>
                            </div>
                        </td>
                        ...
                        <td class="px-6 py-4">
                            <div class="flex items-center gap-2">
                                ...
                                <div>
                                    <p class="text-gray-700">{{ $donor->created_at->format('d M Y') }}</p>
                                    <p class="text-sm text-gray-500">{{ $donor->created_at->format('h:i A') }}</p>
                                </div>
                            </div>
                        </td>
                        ...
                        <td class="px-6 py-4 text-right">
                            <div class="flex justify-end items-center gap-2">
                                <button onclick="openModal({{ $donor->id }})"
                                ...
                                <form action="{{ route('admin.recent-donors.destroy', $donor) }}"
                                      method="POST"
        ...
        <div class="px-6 py-4 border-t border-gray-200 flex items-center justify-between">
            ...
            <div class="flex gap-1">
                ...
                @else
                <a href="{{ $donors->previousPageUrl() }}" class="px-3 py-1 text-gray-700 hover:bg-gray-100 rounded-lg transition">Previous</a>
                @endif
                
                @foreach(range(1, min(5, $donors->lastPage())) as $page)
                    <a href="{{ $donors->url($page) }}" 
                       class="px-3 py-1 rounded-lg transition {{ $donors->currentPage() == $page ? 'bg-blue-600 text-white' : 'text-gray-700 hover:bg-gray-100' }}">
                        {{ $page }}
                    </a>
                @endforeach
                
                @if($donors->hasMorePages())
                <a href="{{ $donors->nextPageUrl() }}" class="px-3 py-1 text-gray-700 hover:bg-gray-100 rounded-lg transition">Next</a>
                @else
...
<script>
    const donors = @json($donors->keyBy('id'));
