            ...
            <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
                @foreach($actions as $action)
                    <a href="{{ route($action['route']) }}"
                        class="group bg-white border border-gray-200 rounded-xl p-4 hover:border-{{ $action['color'] }}-300 hover:shadow-md transition-all duration-300">
                        <div class="flex items-center space-x-4">
                            <div
                                class="w-12 h-12 {{ $action['bg'] }} rounded-lg flex items-center justify-center group-hover:bg-{{ $action['color'] }}-100 transition-colors">
                                <i class="{{ $action['icon'] }} text-{{ $action['color'] }}-600 text-xl"></i>
                            </div>
                            <div>
                                <h3 class="font-semibold text-gray-900 group-hover:text-{{ $action['color'] }}-700">
                                    {{ $action['title'] }}
                                </h3>
                ...
                <div class="space-y-6">
                    ...
                    <div>
                        <div class="flex justify-between items-center mb-2">
                            ...
                            <span
                                class="font-medium {{ $dbStatus === 'Healthy' ? 'text-green-600' : 'text-red-600' }}">{{ $dbStatus }}</span>
                        </div>
                        <div class="w-full bg-gray-200 rounded-full h-2">
                            <div class="{{ $dbStatus === 'Healthy' ? 'bg-green-600' : 'bg-red-600' }} h-2 rounded-full"
                                style="width: {{ $dbPercent }}%"></div>
                        </div>
                    </div>
                    <div>
                        <div class="flex justify-between items-center mb-2">
    ...
    <script>
        document.addEventListener('DOMContentLoaded', function () {
            ...
            new Chart(visitorCtx, {
                type: 'line',
                data: {
                    labels: @json($chartVisitorLabels),
                    datasets: [{
                        label: 'Website Visitors',
                        data: @json($chartVisitorData),
            ...
            new Chart(contentCtx, {
                type: 'doughnut',
                data: {
                    labels: @json($chartContentLabels),
                    datasets: [{
                        data: @json($chartContentData),
                        backgroundColor: [
            ...
            new Chart(staffCtx, {
                type: 'polarArea',
                data: {
                    labels: @json($chartStaffLabels),
                    datasets: [{
                        data: @json($chartStaffData),
                        backgroundColor: [
