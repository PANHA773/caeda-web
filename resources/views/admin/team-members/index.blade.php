        ...
        <div class="bg-white p-5 rounded-2xl border border-gray-100 shadow-sm">
            <div class="flex items-center justify-between">
                <div>
                    ...
                    <p class="text-2xl font-bold text-gray-900 mt-1">{{ $teamMembers->where('is_active', true)->count() }}</p>
                </div>
        ...
        <div class="bg-white p-5 rounded-2xl border border-gray-100 shadow-sm">
            <div class="flex items-center justify-between">
                <div>
                    ...
                    <p class="text-2xl font-bold text-gray-900 mt-1">{{ $teamMembers->where('is_active', false)->count() }}</p>
                </div>
    ...
    <div class="bg-white rounded-2xl border border-gray-200 shadow-sm overflow-hidden">
                    ...
                    <tr class="hover:bg-gray-50 transition-colors duration-150">
                        ...
                        <td class="px-6 py-4">
                            <div class="flex items-center gap-4">
                                <div class="relative">
                                    <img src="{{ $member->photo ?: 'https://ui-avatars.com/api/?name=' . urlencode($member->name) . '&background=random' }}" 
                                         alt="{{ $member->name }}" 
                                ...
                                <div>
                                    <div class="flex items-center gap-2">
                                        ...
                                        <span class="text-xs px-2 py-0.5 bg-blue-100 text-blue-700 rounded-full">ID: {{ $member->id }}</span>
                                    </div>
                                    <p class="text-sm text-gray-500 mt-0.5">{{ $member->email ?? 'No email provided' }}</p>
                                </div>
                            </div>
                        </td>
        ...
        <div class="px-6 py-4 border-t border-gray-200 flex items-center justify-between">
            ...
            <div class="flex gap-1">
                ...
                @else
                <a href="{{ $teamMembers->previousPageUrl() }}" class="px-3 py-1 text-gray-700 hover:bg-gray-100 rounded-lg transition">Previous</a>
                @endif
                
                @foreach(range(1, min(5, $teamMembers->lastPage())) as $page)
                    <a href="{{ $teamMembers->url($page) }}" 
                       class="px-3 py-1 rounded-lg transition {{ $teamMembers->currentPage() == $page ? 'bg-blue-600 text-white' : 'text-gray-700 hover:bg-gray-100' }}">
                        {{ $page }}
                    </a>
                @endforeach
                
                @if($teamMembers->hasMorePages())
                <a href="{{ $teamMembers->nextPageUrl() }}" class="px-3 py-1 text-gray-700 hover:bg-gray-100 rounded-lg transition">Next</a>
                @else
