    ...
    <div class="flex flex-col md:flex-row md:items-center justify-between mb-8">
        ...
        <div class="flex items-center space-x-4 mt-4 md:mt-0">
            <div class="bg-blue-50 px-4 py-3 rounded-lg">
                ...
                <p class="text-2xl font-bold text-gray-800">{{ $leaderTeams->count() }}</p>
            </div>
            <div class="bg-green-50 px-4 py-3 rounded-lg">
                <span class="text-sm text-gray-600">Active</span>
                <p class="text-2xl font-bold text-gray-800">{{ $leaderTeams->where('is_active', true)->count() }}</p>
            </div>
        </div>
    </div>
    ...
    <div class="bg-white rounded-2xl shadow-lg border border-gray-200 mb-6 p-6">
        <div class="flex flex-col md:flex-row md:items-center justify-between gap-4">
            ...
            <div class="flex items-center space-x-3">
                <a href="{{ route('admin.leader-teams.create') }}" 
        ...
        <div class="overflow-x-auto">
            <table class="min-w-full divide-y divide-gray-200">
                ...
                <tbody class="bg-white divide-y divide-gray-200">
                    @forelse($leaderTeams as $index => $member)
                        <tr class="hover:bg-gray-50 transition-colors duration-150">
                                    ...
                                    <div class="flex-shrink-0 h-12 w-12 relative">
                                        ...
                                        @else
                                            <div class="h-12 w-12 bg-gradient-to-br from-blue-100 to-indigo-100 rounded-xl flex items-center justify-center">
                                                <span class="text-blue-600 font-bold text-lg">{{ strtoupper(substr($member->name, 0, 1)) }}</span>
                                            </div>
                                        @endif
                                    </div>
                                    <div class="ml-4">
                            ...
                            <td class="px-6 py-4">
                                <div class="flex items-center">
                                    <div class="h-8 w-8 bg-gray-100 rounded-lg flex items-center justify-center">
                                        <span class="font-bold text-gray-700">{{ $member->order ?? '-' }}</span>
                                    </div>
                                </div>
                            </td>
                            ...
                            <td class="px-6 py-4 whitespace-nowrap">
                                <div class="flex items-center">
                                    <span class="inline-flex items-center px-3 py-1 rounded-full text-sm font-medium {{ $member->is_active ? 'bg-green-100 text-green-800' : 'bg-red-100 text-red-800' }}">
                                        @if($member->is_active)
                            ...
                            <td class="px-6 py-4 whitespace-nowrap text-sm font-medium">
                                <div class="flex items-center space-x-2">
                                    <a href="{{ route('admin.leader-teams.edit', $member->id) }}" 
                                    ...
                                    <form action="{{ route('admin.leader-teams.destroy', $member->id) }}" 
                                          method="POST" 
                                          class="inline-block"
            ...
            <div class="bg-gradient-to-r from-yellow-50 to-amber-50 rounded-2xl p-6 border border-yellow-100">
                <div class="flex items-center">
                    ...
                    <div>
                        ...
                        <p class="text-2xl font-bold text-gray-800">{{ $leaderTeams->where('is_active', false)->count() }}</p>
                    </div>
                </div>
            </div>
            ...
            <div class="bg-gradient-to-r from-purple-50 to-violet-50 rounded-2xl p-6 border border-purple-100">
                <div class="flex items-center">
                    ...
                    <div>
                        ...
                        <p class="text-2xl font-bold text-gray-800">{{ $leaderTeams->max('order') ?? $leaderTeams->count() }}</p>
                    </div>
                </div>
            </div>
        </div>
