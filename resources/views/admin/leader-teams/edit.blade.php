...
@section('title', 'Edit Leader Team')
...
<div class="p-6">
    ...
    <div class="flex flex-col sm:flex-row sm:items-center justify-between mb-8">
        <div>
            <nav class="flex mb-4" aria-label="Breadcrumb">
                <ol class="inline-flex items-center space-x-1 md:space-x-3">
                    <li class="inline-flex items-center">
                        <a href="{{ route('admin.leader-teams.index') }}" class="inline-flex items-center text-sm font-medium text-gray-700 hover:text-blue-600">
            ...
            <p class="text-gray-600 mt-2">Update details for {{ $leaderTeam->name }}</p>
        </div>
    ...
    <div class="grid grid-cols-1 lg:grid-cols-3 gap-6">
        ...
        <div class="lg:col-span-2">
            <div class="bg-white rounded-2xl shadow-lg border border-gray-200 overflow-hidden">
                ...
                <div class="p-6">
                    <form action="{{ route('admin.leader-teams.update', $leaderTeam->id) }}" 
                          method="POST" 
                        ...
                        <div class="space-y-6">
                            ...
                            <div>
                                ...
                                <div class="relative">
                                    ...
                                    <input type="text" 
                                           name="name"
                                           value="{{ old('name', $leaderTeam->name) }}"
                                           class="pl-10 pr-4 py-3 w-full border border-gray-300 rounded-lg focus:ring-2 focus:ring-blue-500 focus:border-blue-500 @error('name') border-red-500 @enderror"
                            ...
                            <div>
                                ...
                                <div class="relative">
                                    ...
                                    <input type="text" 
                                           name="position"
                                           value="{{ old('position', $leaderTeam->position) }}"
                                           class="pl-10 pr-4 py-3 w-full border border-gray-300 rounded-lg focus:ring-2 focus:ring-blue-500 focus:border-blue-500 @error('position') border-red-500 @enderror"
                            ...
                            <div>
                                ...
                                <div class="flex items-center space-x-4">
                                    <div class="relative">
                                        <img src="{{ asset('storage/'.$leaderTeam->image) }}" 
                            ...
                            <div>
                                <label class="block text-sm font-medium text-gray-700 mb-2">
                                    {{ $leaderTeam->image ? 'Change Image' : 'Upload Image' }}
                                </label>
                                <div class="mt-1 flex items-center space-x-4">
                                    ...
                                    <div id="uploadArea" class="flex-1">
                                        <div class="border-2 border-dashed border-gray-300 rounded-xl p-6 text-center hover:border-blue-400 transition-colors cursor-pointer" onclick="document.getElementById('imageInput').click()">
                                            ...
                                            <p class="mt-2 text-sm text-gray-600">Click to {{ $leaderTeam->image ? 'change' : 'upload' }} image</p>
                            ...
                            <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                                ...
                                <div>
                                    ...
                                    <div class="relative">
                                        ...
                                        <input type="text" 
                                               name="gradient"
                                               value="{{ old('gradient', $leaderTeam->gradient) }}"
                                               class="pl-10 pr-4 py-3 w-full border border-gray-300 rounded-lg focus:ring-2 focus:ring-blue-500 focus:border-blue-500 @error('gradient') border-red-500 @enderror"
                                ...
                                <div>
                                    ...
                                    <div class="relative">
                                        ...
                                        <input type="number" 
                                               name="order"
                                               value="{{ old('order', $leaderTeam->order ?? 0) }}"
                                               min="0"
                            ...
                            <div class="flex items-center justify-between p-4 bg-gray-50 rounded-xl border border-gray-200">
                                ...
                                <label class="relative inline-flex items-center cursor-pointer">
                                    <input type="checkbox" 
                                           ...
                                           value="1"
                                           class="sr-only peer"
                                           {{ old('is_active', $leaderTeam->is_active) ? 'checked' : '' }}>
                                    <div class="w-11 h-6 bg-gray-200 peer-focus:outline-none peer-focus:ring-4 peer-focus:ring-blue-300 rounded-full peer peer-checked:after:translate-x-full peer-checked:after:border-white after:content-[''] after:absolute after:top-[2px] after:left-[2px] after:bg-white after:border-gray-300 after:border after:rounded-full after:h-5 after:w-5 after:transition-all peer-checked:bg-blue-600"></div>
                                    <span class="ml-3 text-sm font-medium text-gray-900">
                                        <span id="statusText">{{ $leaderTeam->is_active ? 'Active' : 'Inactive' }}</span>
                                    </span>
                                </label>
                            </div>
        ...
        <div class="space-y-6">
                    ...
                    <div class="text-center">
                        <div class="mb-4">
                            ...
                            @else
                                <div class="h-24 w-24 mx-auto bg-gradient-to-br from-blue-100 to-indigo-100 rounded-xl flex items-center justify-center">
                                    <span class="text-blue-600 font-bold text-3xl">{{ strtoupper(substr($leaderTeam->name, 0, 1)) }}</span>
                                </div>
                            @endif
                        </div>
                        <h3 class="text-xl font-bold text-gray-900 mb-1">{{ $leaderTeam->name }}</h3>
                        <p class="text-gray-600 mb-4">{{ $leaderTeam->position ?? 'Position' }}</p>
                        
                        <div class="inline-flex items-center px-3 py-1 rounded-full text-sm font-medium {{ $leaderTeam->is_active ? 'bg-green-100 text-green-800' : 'bg-red-100 text-red-800' }} mb-4">
                            @if($leaderTeam->is_active)
                        ...
                        <div class="mt-6 p-4 bg-gray-50 rounded-lg">
                            ...
                            <div class="grid grid-cols-2 gap-2 text-sm">
                                ...
                                <div class="font-medium">{{ $leaderTeam->order ?? '0' }}</div>
                                <div class="text-gray-700">Status:</div>
                                <div class="font-medium">{{ $leaderTeam->is_active ? 'Active' : 'Inactive' }}</div>
                                @if($leaderTeam->gradient)
                                    <div class="text-gray-700">Gradient:</div>
                                    <div class="font-medium truncate">{{ $leaderTeam->gradient }}</div>
                                @endif
                            </div>
                        </div>
                    </div>
                    ...
                    <div class="text-center">
                        <div class="mb-4">
                            <div id="previewAvatar" class="h-24 w-24 mx-auto rounded-xl flex items-center justify-center text-3xl font-bold">
                                @if($leaderTeam->image)
                                    <img id="previewCurrentImage" src="{{ asset('storage/'.$leaderTeam->image) }}" class="h-24 w-24 rounded-xl object-cover">
                                @else
                                    <span id="avatarInitials">{{ strtoupper(substr($leaderTeam->name, 0, 1)) }}</span>
                                @endif
                            </div>
                        </div>
                        <h3 id="previewName" class="text-xl font-bold text-gray-900 mb-1">{{ $leaderTeam->name }}</h3>
                        <p id="previewPosition" class="text-gray-600 mb-4">{{ $leaderTeam->position ?? 'Position' }}</p>
                        ...
                        <div class="mt-6 p-4 bg-gray-50 rounded-lg">
                            ...
                            <div class="grid grid-cols-2 gap-2 text-sm">
                                <div class="text-gray-700">Order:</div>
                                <div class="font-medium" id="previewOrder">{{ $leaderTeam->order ?? '0' }}</div>
                                <div class="text-gray-700">Status:</div>
                                <div class="font-medium" id="previewStatusText">{{ $leaderTeam->is_active ? 'Active' : 'Inactive' }}</div>
                            </div>
                        </div>
                    </div>
                </div>
            ...
            <div class="bg-white rounded-2xl shadow-lg border border-red-200 border-t-4 border-t-red-500">
                ...
                <div class="p-6">
                    ...
                    <form action="{{ route('admin.leader-teams.destroy', $leaderTeam->id) }}" 
                          method="POST" 
...
document.addEventListener('DOMContentLoaded', function() {
    ...
    const hasExistingImage = {{ $leaderTeam->image ? 'true' : 'false' }};
    const existingImageUrl = '{{ $leaderTeam->image ? asset("storage/".$leaderTeam->image) : "" }}';
...
function removeImage() {
    ...
    if (previewCurrentImage && '{{ $leaderTeam->image }}') {
        previewCurrentImage.src = '{{ asset("storage/".$leaderTeam->image) }}';
        previewCurrentImage.classList.remove('hidden');
    }
