...
@section('page-title', 'Manage Hero Sections')
@section('page-description', 'List, create, edit, and delete hero sections.')
        ...
        <div class="bg-white p-5 rounded-2xl border border-gray-100 shadow-sm">
            <div class="flex items-center justify-between">
                <div>
                    ...
                    <p class="text-2xl font-bold text-gray-900 mt-1">{{ $heroes->count() }}</p>
                </div>
        ...
        <div class="bg-white p-5 rounded-2xl border border-gray-100 shadow-sm">
            <div class="flex items-center justify-between">
                <div>
                    ...
                    <p class="text-2xl font-bold text-gray-900 mt-1">{{ $heroes->where('is_active', true)->count() }}</p>
                </div>
        ...
        <div class="bg-white p-5 rounded-2xl border border-gray-100 shadow-sm">
            <div class="flex items-center justify-between">
                <div>
                    ...
                    <p class="text-2xl font-bold text-gray-900 mt-1">{{ $heroes->whereNotNull('image')->count() }}</p>
                </div>
        ...
        <div class="bg-white p-5 rounded-2xl border border-gray-100 shadow-sm">
            <div class="flex items-center justify-between">
                <div>
                    ...
                    <p class="text-2xl font-bold text-gray-900 mt-1">{{ $heroes->where('is_active', false)->count() }}</p>
                </div>
    ...
    <div class="bg-white rounded-2xl border border-gray-200 shadow-sm overflow-hidden">
                    ...
                    <tr class="hover:bg-gray-50 transition-colors duration-150">
                        ...
                        <td class="px-6 py-4">
                            <div class="flex items-center gap-4">
                                ...
                                <div>
                                    <div class="flex items-center gap-2">
                                        <h3 class="font-semibold text-gray-900">{{ $hero->title ?: 'No Title' }}</h3>
                                        <span class="text-xs px-2 py-0.5 bg-gray-100 text-gray-700 rounded-full">#{{ $loop->iteration }}</span>
                                    </div>
                                    <div class="flex items-center gap-2 mt-1">
                                        <svg class="w-4 h-4 text-gray-400" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                                            <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M7 7h.01M7 3h5c.512 0 1.024.195 1.414.586l7 7a2 2 0 010 2.828l-7 7a2 2 0 01-2.828 0l-7-7A1.994 1.994 0 013 12V7a4 4 0 014-4z"/>
                                        </svg>
                                        <p class="text-sm text-gray-500">{{ $hero->badge_text ?: 'No Badge' }}</p>
                                    </div>
                                </div>
                            </div>
                        </td>
                        ...
                        <td class="px-6 py-4">
                            <div class="space-y-1">
                                <div class="flex items-center gap-2">
                                    ...
                                    <span class="text-sm font-medium text-gray-700 line-clamp-1 max-w-xs">{{ $hero->highlight_title ?: 'No Highlight' }}</span>
                                </div>
                                <p class="text-xs text-gray-500 line-clamp-2 max-w-xs">{{ $hero->subtitle ?: 'No Subtitle' }}</p>
                            </div>
                        </td>
                        ...
                        <td class="px-6 py-4">
                            <div class="flex items-center gap-3">
                                ...
                                @endif
                                <div>
                                    <p class="text-sm text-gray-600">{{ $hero->image ? 'Image' : 'No Image' }}</p>
                        ...
                        <td class="px-6 py-4 text-right">
                            <div class="flex justify-end items-center gap-2">
                                ...
                                <form action="{{ route('admin.heroes.destroy', $hero->id) }}"
                                      method="POST"
        ...
        <div class="px-6 py-4 border-t border-gray-200 flex items-center justify-between">
            ...
            <div class="flex gap-1">
                ...
                @else
                <a href="{{ $heroes->previousPageUrl() }}" class="px-3 py-1 text-gray-700 hover:bg-gray-100 rounded-lg transition">Previous</a>
                @endif
                
                @foreach(range(1, min(5, $heroes->lastPage())) as $page)
                    <a href="{{ $heroes->url($page) }}" 
                       class="px-3 py-1 rounded-lg transition {{ $heroes->currentPage() == $page ? 'bg-blue-600 text-white' : 'text-gray-700 hover:bg-gray-100' }}">
                        {{ $page }}
                    </a>
                @endforeach
                
                @if($heroes->hasMorePages())
                <a href="{{ $heroes->nextPageUrl() }}" class="px-3 py-1 text-gray-700 hover:bg-gray-100 rounded-lg transition">Next</a>
                @else
