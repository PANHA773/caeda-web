        ...
        <div class="bg-white p-5 rounded-2xl border border-gray-100 shadow-sm">
            <div class="flex items-center justify-between">
                <div>
                    ...
                    <p class="text-2xl font-bold text-gray-900 mt-1">{{ $managers->count() }}</p>
                </div>
        ...
        <div class="bg-white p-5 rounded-2xl border border-gray-100 shadow-sm">
            <div class="flex items-center justify-between">
                <div>
                    ...
                    <p class="text-2xl font-bold text-gray-900 mt-1">{{ $managers->where('position', 'Senior Manager')->count() }}</p>
                </div>
        ...
        <div class="bg-white p-5 rounded-2xl border border-gray-100 shadow-sm">
            <div class="flex items-center justify-between">
                <div>
                    ...
                    <p class="text-2xl font-bold text-gray-900 mt-1">{{ $managers->where('position', 'Assistant Manager')->count() }}</p>
                </div>
        ...
        <div class="bg-white p-5 rounded-2xl border border-gray-100 shadow-sm">
            <div class="flex items-center justify-between">
                <div>
                    ...
                    <p class="text-2xl font-bold text-gray-900 mt-1">#{{ $managers->min('order') ?? 0 }}</p>
                </div>
    ...
    <div class="flex items-center gap-4">
            ...
            @endif
                        ...
                        <td class="px-6 py-4">
                            <div class="flex items-center gap-2">
                                ...
                                @endphp
                                <div class="p-2 {{ $positionIcon }} rounded-lg">
                                    <svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                                        <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M21 13.255A23.931 23.931 0 0112 15c-3.183 0-6.22-.62-9-1.745M16 6V4a2 2 0 00-2-2h-4a2 2 0 00-2 2v2m4 6h.01M5 20h14a2 2 0 002-2V8a2 2 0 00-2-2H5a2 2 0 00-2 2v10a2 2 0 002 2z"/>
                                    </svg>
                                </div>
                                <span class="text-gray-700">{{ $manager->position }}</span>
                            </div>
                        </td>
                        ...
                        <td class="px-6 py-4 text-right">
                            <div class="flex justify-end items-center gap-2">
                                <a href="{{ route('admin.office-managers.edit', $manager->id) }}"
                                ...
                                <form action="{{ route('admin.office-managers.destroy', $manager->id) }}"
                                      method="POST"
                                      onsubmit="return confirm('Are you sure you want to delete {{ $manager->name }}?');">
                                    @csrf
                                    @method('DELETE')
                                    <button type="submit"
        ...
        <div class="px-6 py-4 border-t border-gray-200 flex items-center justify-between">
            ...
            <div class="flex gap-1">
                ...
                @else
                <a href="{{ $managers->previousPageUrl() }}" class="px-3 py-1 text-gray-700 hover:bg-gray-100 rounded-lg transition">Previous</a>
                @endif
                
                @foreach(range(1, min(5, $managers->lastPage())) as $page)
                    <a href="{{ $managers->url($page) }}" 
                       class="px-3 py-1 rounded-lg transition {{ $managers->currentPage() == $page ? 'bg-blue-600 text-white' : 'text-gray-700 hover:bg-gray-100' }}">
                        {{ $page }}
                    </a>
                @endforeach
                
                @if($managers->hasMorePages())
                <a href="{{ $managers->nextPageUrl() }}" class="px-3 py-1 text-gray-700 hover:bg-gray-100 rounded-lg transition">Next</a>
                @else
