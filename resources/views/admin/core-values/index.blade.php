        ...
        <div class="bg-white p-5 rounded-2xl border border-gray-100 shadow-sm">
            <div class="flex items-center justify-between">
                <div>
                    ...
                    <p class="text-2xl font-bold text-gray-900 mt-1">{{ $coreValues->count() }}</p>
                </div>
        ...
        <div class="bg-white p-5 rounded-2xl border border-gray-100 shadow-sm">
            <div class="flex items-center justify-between">
                <div>
                    ...
                    <p class="text-2xl font-bold text-gray-900 mt-1">{{ $coreValues->where('is_active', true)->count() }}</p>
                </div>
        ...
        <div class="bg-white p-5 rounded-2xl border border-gray-100 shadow-sm">
            <div class="flex items-center justify-between">
                <div>
                    ...
                    <p class="text-2xl font-bold text-gray-900 mt-1">{{ $coreValues->where('is_active', false)->count() }}</p>
                </div>
        ...
        <div class="bg-white p-5 rounded-2xl border border-gray-100 shadow-sm">
            <div class="flex items-center justify-between">
                <div>
                    ...
                    <p class="text-2xl font-bold text-gray-900 mt-1">#{{ $coreValues->min('order') ?? 0 }}</p>
                </div>
    ...
    <div class="bg-white rounded-2xl border border-gray-200 shadow-sm overflow-hidden">
                    ...
                    <tr class="hover:bg-gray-50 transition-colors duration-150">
                        ...
                        <td class="px-6 py-4">
                            <div class="flex items-center gap-4">
                                ...
                                <div>
                                    <div class="flex items-center gap-2">
                                        <h3 class="font-semibold text-gray-900">{{ $value->name }}</h3>
                                        <span class="text-xs px-2 py-0.5 bg-gray-100 text-gray-700 rounded-full">ID: {{ $value->id }}</span>
                                    </div>
                                    @if($value->description)
                                        <p class="text-sm text-gray-500 mt-1 line-clamp-2">{{ Str::limit($value->description, 80) }}</p>
                                    @endif
                                </div>
                            </div>
                        </td>
                        ...
                        <td class="px-6 py-4 text-right">
                            <div class="flex justify-end items-center gap-2">
                                ...
                                <form action="{{ route('admin.core-values.destroy', $value->id) }}"
                                      method="POST"
        ...
        <div class="px-6 py-4 border-t border-gray-200 flex items-center justify-between">
            <div class="text-sm text-gray-500">
                Showing {{ $coreValues->firstItem() }} to {{ $coreValues->lastItem() }} of {{ $coreValues->total() }} results
            </div>
            <div class="flex gap-1">
                @if($coreValues->onFirstPage())
                <span class="px-3 py-1 text-gray-400 bg-gray-100 rounded-lg cursor-not-allowed">Previous</span>
                @else
                <a href="{{ $coreValues->previousPageUrl() }}" class="px-3 py-1 text-gray-700 hover:bg-gray-100 rounded-lg transition">Previous</a>
                @endif
                
                @foreach(range(1, min(5, $coreValues->lastPage())) as $page)
                    <a href="{{ $coreValues->url($page) }}" 
                       class="px-3 py-1 rounded-lg transition {{ $coreValues->currentPage() == $page ? 'bg-blue-600 text-white' : 'text-gray-700 hover:bg-gray-100' }}">
                        {{ $page }}
                    </a>
                @endforeach
                
                @if($coreValues->hasMorePages())
                <a href="{{ $coreValues->nextPageUrl() }}" class="px-3 py-1 text-gray-700 hover:bg-gray-100 rounded-lg transition">Next</a>
                @else
