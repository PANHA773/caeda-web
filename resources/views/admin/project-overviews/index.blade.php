...
@section('title', 'Project Overviews')
...
<div class="container mx-auto px-4 sm:px-6 lg:px-8 py-8">
    ...
    <div class="mb-8">
        <div class="flex flex-col sm:flex-row sm:items-center sm:justify-between mb-6">
            ...
            <a href="{{ route('admin.project-overviews.create') }}"
            ...
            <div class="bg-gradient-to-r from-blue-50 to-blue-100 rounded-xl p-5 border border-blue-200">
                <div class="flex items-center">
                    ...
                    <div>
                        ...
                        <p class="text-2xl font-bold text-gray-900">{{ $projectOverviews->count() }}</p>
                    </div>
                </div>
            </div>
            ...
            <div class="bg-gradient-to-r from-green-50 to-green-100 rounded-xl p-5 border border-green-200">
                <div class="flex items-center">
                    ...
                    <div>
                        ...
                        <p class="text-2xl font-bold text-gray-900">{{ $projectOverviews->where('is_active', true)->count() }}</p>
                    </div>
                </div>
            </div>
            ...
            <div class="bg-gradient-to-r from-purple-50 to-purple-100 rounded-xl p-5 border border-purple-200">
                <div class="flex items-center">
                    ...
                    <div>
                        ...
                        <p class="text-2xl font-bold text-gray-900">{{ $projectOverviews->first() ? $projectOverviews->first()->created_at->format('M d') : 'N/A' }}</p>
                    </div>
                </div>
            </div>
            ...
            <div class="bg-gradient-to-r from-amber-50 to-amber-100 rounded-xl p-5 border border-amber-200">
                <div class="flex items-center">
                    ...
                    <div>
                        ...
                        <p class="text-2xl font-bold text-gray-900">{{ $projectOverviews->first() ? '#' . $projectOverviews->first()->id : 'N/A' }}</p>
                    </div>
                </div>
            </div>
        </div>
    ...
    <div class="bg-white rounded-2xl shadow-lg border border-gray-100 overflow-hidden">
        ...
        <div class="overflow-x-auto">
            <table class="min-w-full divide-y divide-gray-200">
                ...
                <tbody class="bg-white divide-y divide-gray-100">
                    @forelse($projectOverviews as $index => $overview)
                    <tr class="hover:bg-gray-50/50 transition-colors duration-150">
                        ...
                        <td class="px-6 py-5">
                            <div class="flex flex-col space-y-1">
                                <h4 class="font-semibold text-gray-900 text-base">{{ $overview->title }}</h4>
                                <div class="flex items-center space-x-2 mt-1">
                                    <span class="text-xs px-2 py-1 bg-gray-100 text-gray-700 rounded">ID: {{ $overview->id }}</span>
                                    <span class="text-xs text-gray-500">{{ $overview->created_at->diffForHumans() }}</span>
                                </div>
                            </div>
                        </td>
                        <td class="px-6 py-5">
                            <div class="text-sm text-gray-600 line-clamp-2 max-w-md">
                                {!! \Illuminate\Support\Str::limit(strip_tags($overview->description), 120) !!}
                            </div>
                        </td>
                        <td class="px-6 py-5 whitespace-nowrap">
                        ...
                        <td class="px-6 py-5 whitespace-nowrap">
                            <div class="flex items-center space-x-3">
                                <a href="{{ route('admin.project-overviews.edit', $overview->id) }}" 
                                ...
                                <form action="{{ route('admin.project-overviews.destroy', $overview->id) }}"
                                      method="POST"
                                      class="inline-block"
        ...
        @if($projectOverviews->hasPages())
        <div class="px-6 py-4 border-t border-gray-200 bg-gray-50/50">
            <div class="flex items-center justify-between">
                <div class="text-sm text-gray-700">
                    Showing {{ $projectOverviews->firstItem() }} to {{ $projectOverviews->lastItem() }} of {{ $projectOverviews->total() }} results
                </div>
                <div class="flex space-x-2">
                    {{ $projectOverviews->links() }}
                </div>
            </div>
        </div>
        @endif
