...
@section('title', 'Edit Project Overview')
...
<div class="container mx-auto px-4 sm:px-6 lg:px-8 py-8">
    ...
    <div class="mb-8">
        <div class="flex flex-col sm:flex-row sm:items-center sm:justify-between mb-6">
            <div>
                <nav class="flex items-center space-x-2 text-sm text-gray-500 mb-3">
                    <a href="{{ route('admin.project-overviews.index') }}" class="hover:text-blue-600 transition">Project Overviews</a>
                    <svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                        <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M9 5l7 7-7 7"></path>
                    </svg>
                    <span class="text-gray-400">Edit #{{ $projectOverview->id }}</span>
                </nav>
                <div class="flex items-center">
                    ...
                    <div>
                        ...
                        <p class="mt-1 text-sm text-gray-600">Update details for "{{ $projectOverview->title }}"</p>
                    </div>
                </div>
            </div>
        ...
        <div class="grid grid-cols-1 md:grid-cols-4 gap-4 mb-8">
            <div class="bg-gradient-to-r from-blue-50 to-blue-100 rounded-xl p-4 border border-blue-200">
                ...
                <p class="text-lg font-semibold text-gray-900">{{ $projectOverview->created_at->format('M d, Y') }}</p>
            </div>
            <div class="bg-gradient-to-r from-green-50 to-green-100 rounded-xl p-4 border border-green-200">
                <p class="text-sm font-medium text-gray-600">Status</p>
                <div class="flex items-center">
                    @if($projectOverview->is_active)
            ...
            <div class="bg-gradient-to-r from-purple-50 to-purple-100 rounded-xl p-4 border border-purple-200">
                ...
                <p class="text-lg font-semibold text-gray-900">{{ strlen($projectOverview->description) }}</p>
            </div>
            <div class="bg-gradient-to-r from-amber-50 to-amber-100 rounded-xl p-4 border border-amber-200">
                <p class="text-sm font-medium text-gray-600">Last Updated</p>
                <p class="text-lg font-semibold text-gray-900">{{ $projectOverview->updated_at->diffForHumans() }}</p>
            </div>
        </div>
    </div>
    ...
    <div class="max-w-4xl mx-auto">
        <div class="bg-gradient-to-br from-white via-white to-gray-50 rounded-2xl shadow-xl border border-gray-100 overflow-hidden">
            ...
            {{-- Form Content --}}
            <form action="{{ route('admin.project-overviews.update', $projectOverview->id) }}" method="POST" class="p-8">
                @csrf
                @method('PUT')
                ...
                <div class="space-y-6">
                    ...
                    <div class="relative">
                        ...
                        <div class="relative">
                            <input type="text"
                                   name="title"
                                   value="{{ old('title', $projectOverview->title) }}"
                    ...
                    <div>
                        ...
                        <textarea name="description"
                                  rows="8"
                                  class="w-full px-4 py-3 border border-gray-300 rounded-lg focus:outline-none focus:ring-2 focus:ring-blue-500 focus:border-transparent transition-all bg-white hover:border-gray-400">{{ old('description', $projectOverview->description) }}</textarea>
                        <div class="flex justify-between mt-1">
                            <span class="text-xs text-gray-500">Provide a detailed overview of the project</span>
                            <span class="text-xs text-gray-400" id="charCount">{{ strlen($projectOverview->description) }} characters</span>
                        </div>
                    </div>
                        ...
                        <div class="flex items-center space-x-4 p-4 bg-gray-50 rounded-lg border border-gray-200">
                            <div class="relative">
                                <input type="checkbox"
                                       ...
                                       id="is_active"
                                       value="1"
                                       {{ old('is_active', $projectOverview->is_active) ? 'checked' : '' }}
                                       class="sr-only peer">
                            ...
                            <div>
                                <span class="text-sm font-medium text-gray-900" id="statusText">{{ $projectOverview->is_active ? 'Active' : 'Hidden' }}</span>
                ...
                <div class="mt-10 pt-8 border-t border-gray-200">
                    <div class="flex flex-col sm:flex-row sm:items-center sm:justify-between space-y-4 sm:space-y-0">
                        ...
                        <div class="flex space-x-4">
                            ...
                            <a href="{{ route('admin.project-overviews.create') }}" class="px-6 py-3 bg-gradient-to-r from-green-600 to-emerald-600 text-white font-medium rounded-lg hover:from-green-700 hover:to-emerald-700 transition-all shadow-sm hover:shadow-md flex items-center">
                                <svg class="w-5 h-5 mr-2" fill="none" stroke="currentColor" viewBox="0 0 24 24">
        ...
        <div class="mt-8 bg-gradient-to-br from-white via-white to-gray-50 rounded-2xl shadow-lg border border-red-200 overflow-hidden">
            ...
            <div class="p-8">
                <div class="flex flex-col md:flex-row md:items-center md:justify-between">
                    ...
                    <form action="{{ route('admin.project-overviews.destroy', $projectOverview->id) }}" 
                          method="POST"
                          onsubmit="return confirm('⚠️ Are you absolutely sure? This will permanently delete the project overview \'{{ $projectOverview->title }}\' and cannot be undone.');">
                        @csrf
                        @method('DELETE')
