...
@section('content')
    <div class="container mx-auto p-6">
        ...
        <form action="{{ route('admin.welcome_sections.store') }}" method="POST" enctype="multipart/form-data"
            ...
            <div>
                ...
                <textarea name="description[]" rows="3" class="w-full border rounded px-3 py-2 mb-2"
                    placeholder="Paragraph 1">{{ old('description.0') }}</textarea>
                <textarea name="description[]" rows="3" class="w-full border rounded px-3 py-2 mb-2"
                    placeholder="Paragraph 2">{{ old('description.1') }}</textarea>
                <textarea name="description[]" rows="3" class="w-full border rounded px-3 py-2"
                    placeholder="Paragraph 3">{{ old('description.2') }}</textarea>
            </div>
                ...
                <div class="space-y-2">
                    @for($i = 0; $i < 3; $i++)
                        <div class="flex gap-2">
                            <input type="text" name="badges[{{ $i }}][text]" placeholder="Badge Text"
                                value="{{ old("badges.$i.text") }}" class="border rounded px-2 py-1 w-1/2">
                            <input type="color" name="badges[{{ $i }}][color]" value="{{ old("badges.$i.color", '#facc15') }}"
            ...
            <div>
                ...
                <div class="space-y-2">
                    @for($i = 0; $i < 3; $i++)
                        <div class="flex gap-2">
                            <input type="text" name="stats[{{ $i }}][number]" placeholder="Number"
                                value="{{ old("stats.$i.number") }}" class="border rounded px-2 py-1 w-1/2">
                            <input type="text" name="stats[{{ $i }}][label]" placeholder="Label"
                                value="{{ old("stats.$i.label") }}" class="border rounded px-2 py-1 w-1/2">
                        </div>
                    @endfor
                </div>
            </div>
