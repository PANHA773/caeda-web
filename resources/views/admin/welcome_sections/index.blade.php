                    ...
                    @foreach($sections as $section)
                        <tr class="hover:bg-gray-50">
                            ...
                            <td class="px-4 py-2 max-w-xs truncate">
                                {{ is_array($section->description) ? implode(' ', $section->description) : $section->description }}
                            </td>
                            <td class="px-4 py-2">
                            ...
                            <td class="px-4 py-2">
                                @if($section->image)
                                    <img src="{{ asset('storage/' . $section->image) }}" alt="Section Image"
                                        class="w-24 h-24 object-cover rounded-xl border-2 border-gray-300">
                                @else
                                    <div class="w-24 h-24 rounded-xl border-2 border-gray-300 bg-gray-100 flex items-center justify-center text-gray-400 text-xs">
                                        No Image
                                    </div>
                                @endif
                            </td>
                            ...
                            <td class="px-4 py-2 space-x-2">
                                ...
                                <form action="{{ route('admin.welcome_sections.destroy', $section) }}" method="POST"
                                    class="inline-block">
                                    @csrf
                                    @method('DELETE')
