    ...
    <div class="bg-white shadow rounded-lg p-6">
        ...
        <form action="{{ route('admin.pricing.update', $plan->id) }}" method="POST">
            ...
            <div class="mb-4">
                ...
                <input type="number" step="0.01" name="monthly_price" value="{{ old('monthly_price', $plan->monthly_price) }}" 
                       class="w-full border border-gray-300 rounded px-3 py-2 focus:outline-none focus:ring-2 focus:ring-blue-500" required>
            </div>
            ...
            <div class="mb-4">
                ...
                <input type="number" step="0.01" name="annual_price" value="{{ old('annual_price', $plan->annual_price) }}" 
                       class="w-full border border-gray-300 rounded px-3 py-2 focus:outline-none focus:ring-2 focus:ring-blue-500" required>
            </div>
            ...
            <div class="mb-4">
                ...
                <textarea name="description" rows="3" 
                          class="w-full border border-gray-300 rounded px-3 py-2 focus:outline-none focus:ring-2 focus:ring-blue-500">{{ old('description', $plan->description) }}</textarea>
            </div>

            {{-- Popular --}}
            <div class="mb-4 flex items-center">
                <input type="checkbox" name="is_popular" id="is_popular" value="1" class="mr-2" {{ old('is_popular', $plan->is_popular) ? 'checked' : '' }}>
                <label for="is_popular" class="text-gray-700 font-medium">Mark as Most Popular</label>
            </div>

            {{-- Active --}}
            <div class="mb-6 flex items-center">
                <input type="checkbox" name="is_active" id="is_active" value="1" class="mr-2" {{ old('is_active', $plan->is_active) ? 'checked' : '' }}>
